...
- **services** -- Generates the full check queue by combining every team with every box and service, and serves checks round-robin to agents. Also holds the competition pause state; while paused, agents are handed no-op assignments instead of real checks.
...
3. If you need new database queries, add them in the relevant sink subpackage.
4. Add swaggo annotations (see below) and regenerate the Swagger docs.

## Swagger / OpenAPI Docs

The API ships with a Swagger UI at `/swagger/`. The spec is generated from source annotations and committed to the repo, so it must be regenerated by hand whenever routes, handler annotations, or the JSON shape of a type crossing the API boundary change.

Install `swag`, then regenerate from `src/`:
...
Because `--pd` parses dependencies, `swag` must be new enough to read the standard library of the Go toolchain in use — an older `swag` fails while parsing stdlib source. If that happens, run the latest without installing it: `go run github.com/swaggo/swag/cmd/swag@latest init --pd --parseInternal -g api/main.go -o ./api/openapi`.
...
Documentation is kept in sync manually after code changes. Claude Code skills in `.claude/skills/` encode the conventions and process:

| Skill | Invocation | Updates |
|-------|------------|---------|
| `update-docs` | `/update-docs` | Module READMEs across all packages, plus the guides in `docs/` and the root README |
| `swagger-docs` | `/swagger-docs` | Swagger annotations on route handlers + regenerates OpenAPI spec |
| `slop-it-up` | `/slop-it-up` | Runs `update-docs`, `swagger-docs`, and `update-tests` in sequence |
