...
The generated spec is committed to the repo and must be regenerated by hand — there is no hook that does it for you. From `src/`:
...
If `swag` fails while parsing a file under the Go toolchain's own `libexec/src` (for example `method must have no type parameters` in `math/rand/v2`), the installed `swag` is older than the Go toolchain. Do not work around it by dropping `--pd` — run the current release instead:

```bash
go run github.com/swaggo/swag/cmd/swag@latest init --pd --parseInternal -g api/main.go -o ./api/openapi
```

Mention the version mismatch in the final report so the installed binary can be upgraded.
